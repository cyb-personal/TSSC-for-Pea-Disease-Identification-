...
&nbsp;  百度网盘链接：通过网盘分享的文件：
链接: https://pan.baidu.com/s/18FxZMhVcK-5hRwAhoJS8mQ 提取码: bq9g 复制这段内容后打开百度网盘手机App，操作更方便哦
...
推荐使用Anaconda创建虚拟环境，确保依赖版本匹配（避免兼容性问题）：
...
&nbsp; --data\_dir ./pea\_disease\_dataset \\  # 数据集根目录（解压后的路径）
...
&nbsp; --log\_interval 10  # 每10个batch打印一次训练日志
...
| `--data\_dir`    | 数据集根目录路径                       | `./pea\_disease\_dataset` |

| `--epochs`      | 训练轮数                              | 60              |

| `--batch\_size`  | 批次大小（根据GPU显存调整，8/16/32）  | 16              |
...
\- 训练过程中，模型会自动保存\*\*验证集宏平均F1值最高\*\*的权重至 `--save\_dir` 目录，文件名为 `best\_tssc.pth`；  

\- 训练日志（损失值、准确率、F1值）会实时打印，并保存至 `train\_log.txt`。  
...
&nbsp; --image\_path ./examples/pea\_mildew.jpg \\  # 输入图像路径（示例图像存于examples/）

&nbsp; --weight\_path ./weights/best\_tssc.pth \\  # 预训练权重路径
...
输入图像路径：./examples/pea\_mildew.jpg
...
├── pea\_disease\_dataset/  # 五类豌豆病害数据集（需从百度网盘下载）

├── examples/             # 预测示例图像（如rust\_example.png）
...
│   ├── split attention.py    #实现split注意力机制

│   ├── three neighbor attention.py   #实现三邻域通道注意力机制

├── dataset/              # 数据处理文件夹

│   ├── data\_loader.py    # 数据集加载与预处理（自动划分训练/验证/测试集）
